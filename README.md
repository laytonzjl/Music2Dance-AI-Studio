...
## 📖 项目概述
![软件主界面](vis.PNG)
**M2D AI Studio** 是一个专为游戏开发人员、动画师及 3D 内容创作者打造的多模态舞蹈动作生成软件。输入音乐（目前最长支持20秒），该软件就能进行高效的舞蹈动作生成，实现从音乐到 BVH 格式的动作资产的一键式转换。
...
3. **音频选择**：尽量选择音质清晰、节拍明显的音频片段。若音频过长，请使用裁剪功能限定 20 秒内的生成范围。
...
## 🚀 Quick Start
1. **Get the App**: Visit the [Releases](https://github.com/laytonzjl/Music2Dance-AI-Studio/releases) page to download the latest installer.
...
* **CPU Inference**: ~~Efficient inference on standard desktop CPUs without the need for GPU acceleration.~~ GPU acceleration switching is now supported!
...
* **Visual Animation**: Supports visualizing generation results in the browser.
...
| **Visual Animation** | Enabled | Visualize generation results in the browser. |
...
* **Dataset**: [FineDance](https://github.com/Finedance-dataset) dance motion benchmark.
* **Human Model**: [SMPL-X](https://github.com/vchoutas/smplx) parametric human model.
